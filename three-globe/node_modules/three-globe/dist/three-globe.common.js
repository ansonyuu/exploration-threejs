...
var HexBinLayerKapsule = Kapsule__default['default']({
  ...
  update: function update(state) {
    ...
    if (state.hexBinMerge) {
      // merge points into a single mesh
      var hexPointsGeometry = !hexBins.length ? new THREE$4.BufferGeometry() : BufferGeometryUtils_js.BufferGeometryUtils.mergeBufferGeometries(hexBins.map(function (d) {
        var obj = d.__threeObj;
        d.__threeObj = undefined; // unbind merged points
    ...
    threeDigest(state.pathsData, state.scene, {
      ...
      updateObj: function updateObj(group, path) {
        ...
        if (!group.children.length || useFatLine === (group.children[0].type === 'Line')) {
          ...
          var _obj = useFatLine ? new threeFatline.Line2(new threeFatline.LineGeometry(), new threeFatline.LineMaterial()) : new THREE$8.Line(new THREE$8.BufferGeometry(), sharedShaderMaterial.clone() // Separate material instance per object to have dedicated uniforms (but shared shaders)
          );
