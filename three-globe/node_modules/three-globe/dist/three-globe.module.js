...
var GlobeLayerKapsule = Kapsule({
  ...
  stateInit: function stateInit() {
    // create globe
    var globeGeometry = new THREE$1.SphereBufferGeometry(GLOBE_RADIUS, 75, 75);
    ...
    var graticulesObj = new THREE$1.LineSegments(new GeoJsonGeometry(geoGraticule10(), GLOBE_RADIUS, 2), new THREE$1.LineBasicMaterial({
      color: 'lightgrey',
      transparent: true,
      opacity: 0.1
    }));
...
var PointsLayerKapsule = Kapsule({
  ...
  update: function update(state) {
    ...
    if (state.pointsMerge) {
      ...
      var pointsGeometry = !state.pointsData.length ? new THREE$2.BufferGeometry() : BufferGeometryUtils.mergeBufferGeometries(state.pointsData.map(function (d) {
...
var applyMatrix4Fn$1 = new THREE$4.BufferGeometry().applyMatrix4 ? 'applyMatrix4' : 'applyMatrix';
var HexBinLayerKapsule = Kapsule({
  ...
  update: function update(state) {
    ...
    if (state.hexBinMerge) {
      // merge points into a single mesh
      var hexPointsGeometry = !hexBins.length ? new THREE$4.BufferGeometry() : BufferGeometryUtils.mergeBufferGeometries(hexBins.map(function (d) {
        var obj = d.__threeObj;
        d.__threeObj = undefined; // unbind merged points
        ...
        var topColor = new THREE$4.Color(topColorAccessor(d));
        var sideColor = new THREE$4.Color(sideColorAccessor(d));
        var nVertices = geom.attributes.position.count;
  ...
  update: function update(state) {
    ...
    threeDigest(singlePolygons, state.scene, {
      ...
      createObj: function createObj() {
        ...
        obj.__defaultSideMaterial = new THREE$5.MeshBasicMaterial({
          side: THREE$5.DoubleSide,
          depthWrite: true
        });
        obj.__defaultCapMaterial = new THREE$5.MeshBasicMaterial({
          side: THREE$5.DoubleSide,
          depthWrite: true
        }); // conic geometry

        obj.add(new THREE$5.Mesh(undefined, [obj.__defaultSideMaterial, // side material
        obj.__defaultCapMaterial // cap material
        ])); // polygon stroke

        obj.add(new THREE$5.Line(undefined, new THREE$5.LineBasicMaterial()));
...
var PathsLayerKapsule = Kapsule({
  ...
  update: function update(state) {
    ...
    var sharedShaderMaterial = new THREE$8.ShaderMaterial(_objectSpread2(_objectSpread2({}, gradientShaders$1), {}, {
      transparent: true,
      blending: THREE$8.AdditiveBlending
    }));
    threeDigest(state.pathsData, state.scene, {
      ...
      updateObj: function updateObj(group, path) {
        ...
        if (!group.children.length || useFatLine === (group.children[0].type === 'Line')) {
          ...
          var _obj = useFatLine ? new Line2(new LineGeometry(), new LineMaterial()) : new THREE$8.Line(new THREE$8.BufferGeometry(), sharedShaderMaterial.clone() // Separate material instance per object to have dedicated uniforms (but shared shaders)
          );
        ...
        if (!useFatLine) {
          ...
          }
          {
            ...
            if (colors instanceof Array) {
              ...
              obj.material.vertexColors = THREE$8.VertexColors;
            } else {
              // single color
              var color = colors;
              var opacity = colorAlpha(color);
              ...
              obj.material.transparent = opacity < 1;
              obj.material.opacity = opacity;
              obj.material.vertexColors = THREE$8.NoColors;
            }
          }
    ...
    function calcColorVertexArray(colors, numSegments) {
      ...
      var vertexColorArray = new THREE$8.Float32BufferAttribute(numVerticesGroup * numArgs * numVerticesPerSegment, numArgs);
...
var TilesLayerKapsule = Kapsule({
  props: {
    ...
    tileMaterial: {
      "default": function _default() {
        return new THREE$9.MeshLambertMaterial({
          color: '#ffbb88',
          opacity: 0.4,
          transparent: true
        });
    ...
    threeDigest(state.tilesData, state.scene, {
      ...
      updateObj: function updateObj(obj, d) {
        ...
        var applyPosition = function applyPosition(td) {
          ...
          obj.geometry = new THREE$9.SphereBufferGeometry(GLOBE_RADIUS * (1 + alt), Math.ceil(width / curvatureResolution), Math.ceil(height / curvatureResolution), deg2Rad(90 - width / 2) + (useGlobeProjection ? rotLng : 0), deg2Rad(width), deg2Rad(90 - height / 2) + (useGlobeProjection ? rotLat : 0), deg2Rad(height));

          if (!useGlobeProjection) {
            // rotate obj instead. order matters, rotate longitudinally first.
            obj.setRotationFromEuler(new THREE$9.Euler(rotLat, rotLng, 0, 'YXZ'));
          }
        };
...
var LabelsLayerKapsule = Kapsule({
  props: {
    ...
    labelTypeFace: {
      ...
      onChange: function onChange(tf, state) {
        state.font = new THREE$a.Font(tf);
      }
    },
    labelColor: {
      "default": function _default() {
  ...
  update: function update(state) {
    ...
    var pxPerDeg = 2 * Math.PI * GLOBE_RADIUS / 360;
    var circleGeometry = new THREE$a.CircleBufferGeometry(1, 16);
    threeDigest(state.labelsData, state.scene, {
      createObj: function createObj() {
        var material = new THREE$a.MeshLambertMaterial();
        var obj = new THREE$a.Group(); // container

        obj.add(new THREE$a.Mesh(circleGeometry, material)); // dot

        obj.add(new THREE$a.Mesh(undefined, material)); // text
      ...
      updateObj: function updateObj(obj, d) {
        ...
        textObj.geometry = new THREE$a.TextBufferGeometry(textAccessor(d), {
          font: state.font,
          size: textHeight,
          height: 0,
        ...
        var applyPosition = function applyPosition(td) {
          ...
          obj.lookAt(state.scene.localToWorld(new THREE$a.Vector3(0, 0, 0))); // face globe (local) center
...
var Globe = Kapsule({
  props: _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({
    rendererSize: {
      "default": new THREE$b.Vector2(window.innerWidth, window.innerHeight),
  ...
  init: function init(threeObj, state, _ref12) {
    ...
    var initGlobe = function initGlobe() {
      if (animateIn) {
        ...
        var rotAxis = new THREE$b.Vector3(0, 1, 0);
        new TWEEN.Tween({
          rot: Math.PI * 2
        }).to({
          rot: 0
